...
public class Decompress {
	...
	private void createHeap() {
		/**
		 * create Heap that contain all bytes in the file in order to use it to
		 * build the tree this method create an array of heaps , keep minimising
		 * it until it size = 1
		 */
	...
	private Huffman[] createHuffTable(String result) {
		/**
		 * create table of all characters and its freq, byte representation ,
		 * ... etc
		 */
	...
	public int search(Counter[] sortedArray, Counter item, int from, int to) {
		/**
		 * search for an item in a sorted array and return its index , or -1 if
		 * not found
		 */
	...
	public Heap<Counter>[] sort(Heap<Counter>[] array) {
		/*
		 * sort heap array we have two arrays of counters (original: have same
		 * order of Counters which occur in received heap[] and (sorted ) which
		 * have the item in original but sorted we sort sorted and then fill
		 * index[] index[] is an array that will contain the right index for
		 * each element in original, in order to have a sorted heap[] each item
		 * in original will be moved to its right index in newHeap[] using
		 * index[]
		 */
